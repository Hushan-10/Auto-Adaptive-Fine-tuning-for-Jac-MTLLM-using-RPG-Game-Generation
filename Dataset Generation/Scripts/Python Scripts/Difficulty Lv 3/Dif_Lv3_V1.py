...
def _largest_component_cells(w: int, h: int, blocked: Set[Tuple[int, int]]) -> List[Tuple[int, int]]:
    """Return the cells of the largest connected component (4-connected) when `blocked` are removed."""
    ...
    for yy in range(h):
        for xx in range(w):
            ...
            if p in blocked or p in seen:
                continue
            comp = _reachable_from(p, w, h, blocked)
            if len(comp) > len(best):
                best = list(comp)
            seen |= comp
    return best

def _connectivity_target_ratio(n: int) -> float:
    """
    Minimum fraction of non-wall tiles that should be reachable from the player.
    Gentle thresholds to preserve existing behavior.
    """
    if n <= 14: return 0.50
    if n <= 16: return 0.55
    if n <= 18: return 0.58
...
def _place_small_obstacles(rng: random.Random, level: Dict[str,Any], walls_set: Set[Tuple[int,int]],
    ...
    max_possible = max(0, len(reach0))  # already includes player cell
    min_reach_cells = min(max_possible, int(base_target * max(1, non_wall_free)))
...
def _place_enemies(rng: random.Random, level: Dict[str,Any], walls_set: Set[Tuple[int,int]],
    ...
    while sep >= 2:
        ...
        if len(picked) >= n:
            result = picked[:n]
            break
        # If we placed at least some, keep the best-so-far while relaxing
    ...
    if len(result) < n:
        ...
        placed = set(result)
        # Keep a weaker separation (≥2) from existing enemies and player.
        soft_sep = 2
        pool = [c for c in candidates if cheb(c, player_cell) >= 2 and c not in placed]
        rng.shuffle(pool)
        for c in pool:
            if len(result) >= n:
                break
...
def build_map(level: Dict[str, Any], rng: random.Random) -> Dict[str, Any]:
    ...
    used: Set[Tuple[Tuple[int,int], Tuple[int,int]]] = set()
    def _norm_key(w):
        a = (w["start_pos"]["x"], w["start_pos"]["y"])
        b = (w["end_pos"]["x"], w["end_pos"]["y"])
        return (a, b) if a <= b else (b, a)
    for wseg in keep:
        used.add(_norm_key(wseg))

    need = level["num_wall"] - keep_n
    new_walls: List[Dict[str, Dict[str, int]]] = []
    for _ in range(max(0, need)):
        seg = _random_segment_within_range(rng, width, height, used, min_len, max_len)
        if seg:
            new_walls.append(seg)
    walls = keep + new_walls

    # Final hard validation: ALL walls length within [min_len, max_len]
    def _validate_wall_list(wlist: List[Dict[str, Dict[str, int]]]) -> List[Dict[str, Dict[str, int]]]:
        validated_local: List[Dict[str, Dict[str, int]]] = []
        used_final: Set[Tuple[Tuple[int,int], Tuple[int,int]]] = set()
        for seg in wlist:
            L = _segment_cells_len(seg)
            if not (min_len <= L <= max_len):
                seg = _refit_segment_to_range(rng, seg, width, height, min_len, max_len, used_final)
                if seg is None:
                    continue
            else:
                key = _norm_key(seg)
                if key in used_final:
                    seg = _random_segment_within_range(rng, width, height, used_final, min_len, max_len)
                    if seg is None:
                        continue
            validated_local.append(seg)
        return validated_local[:level["num_wall"]]
    ...
    def _evaluate_connectivity(wlist: List[Dict[str, Dict[str, int]]]) -> Tuple[float, Set[Tuple[int,int]]]:
        ...
        largest = len(_largest_component_cells(width, height, wc))
        ratio = largest / max(1, non_wall)
        return ratio, wc

    target_ratio = _connectivity_target_ratio(min(width, height))
    best_ratio, best_wall_cells = _evaluate_connectivity(walls)
    best_walls = walls[:]

    if best_ratio < target_ratio:
        tries = 18  # quick refinement; keeps the 'keep' walls fixed
        while tries > 0 and best_ratio < target_ratio:
            tries -= 1
            # Regenerate only the 'new' portion against the current keep set
            used_tmp: Set[Tuple[Tuple[int,int], Tuple[int,int]]] = set(_norm_key(w) for w in keep)
            candidate_new: List[Dict[str, Dict[str, int]]] = []
            for _ in range(max(0, need)):
                seg = _random_segment_within_range(rng, width, height, used_tmp, min_len, max_len)
                if seg:
                    candidate_new.append(seg)
            candidate = _validate_wall_list(keep + candidate_new)
            cand_ratio, cand_wall_cells = _evaluate_connectivity(candidate)
            if cand_ratio > best_ratio:
                best_ratio, best_walls, best_wall_cells = cand_ratio, candidate, cand_wall_cells
                if best_ratio >= target_ratio:
                    break
        walls = best_walls
    ...
    if largest_cells:
        interior = [(x, y) for (x, y) in largest_cells if 0 < x < width - 1 and 0 < y < height - 1]
        px, py = rng.choice(interior or largest_cells)
    else:
    ...
    # Small obstacles: cannot overlap walls or player (and later enemies); keep connectivity healthy
    small_obstacles = _place_small_obstacles(rng, level, wall_cells, player_cell)

    # Enemies: cannot overlap walls, obstacles, or player; must be reachable from player
    ...
    # Final reachability: every enemy reachable from player given walls+obstacles (already enforced)
